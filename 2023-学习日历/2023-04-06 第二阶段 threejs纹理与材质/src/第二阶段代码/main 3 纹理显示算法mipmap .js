...
import dpi from './assets/images/dpi.jpg' //先引入 才能使用load加载
const dpiTexture = textLoader.load(dpi)
...
//++ 当一个文素覆盖小于一个像素时，贴图将如何采样 //默认值是THREE.LinearFilter
dpiTexture.minFilter = THREE.NearestFilter //最接近的纹素值
//++ 当一个文素覆盖大于一个像素时，贴图将如何采样 //默认值是THREE.LinearFilter
dpiTexture.magFilter = THREE.NearestFilter //最接近的纹素值
//
const cubeGeometry2 = new THREE.BoxBufferGeometry(5,5,5)
//纹理材质 map
const cubeMaterial = new THREE.MeshBasicMaterial({color:"pink",map:dpiTexture })
const cube2 = new THREE.Mesh(cubeGeometry2,cubeMaterial)
// cube2.position.set(-30,-8,-15)
console.log(cube2,dpiTexture)
scene.add(cube2)
