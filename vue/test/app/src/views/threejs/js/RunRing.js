...
class RunRing {
  ...
  CreatRing() {
    ...
    const texture = textureLoader.load(this.img);
    ...
    for (let i = 0; i < this.position.length; i += 1) {
      //组合生成模型
      const meshring = new THREE.Mesh(geometry, material2);
    ...
    const meshrings = this.meshrings
    setTimeout(() => {
      setInterval(() => {
        //循环所有创建的模型，进行改变
        for (let i = 0; i < position.length; i += 1) {
          ...
          if (meshrings[i]) {
            ...
            if (meshrings[i].scale.x < 1) {
              meshrings[i].scale.set(
                meshrings[i].scale.x + 0.01,
                meshrings[i].scale.x + 0.01,
                meshrings[i].scale.x + 0.01,
              );
            } else {
              //当模型比例大于1的时候，模型重置
              meshrings[i].scale.set(0.1, 0.1, 0.1);
            }
          }
        }
