...
samllball.add(pointLight) //把灯光添加到小球上 小球作为灯光的父级 

scene.add(samllball) //发光小球添加到场景
// 初始化用户界面 
//改变阴影大小
...
gui.add(pointLight, 'decay').min(0).max(5).step(0.001)
//聚光灯范围大小
//   gui 
//   .add(spotLight, "angle")
//   .min(0)
...
function render() {
    ...
    const speed = 1 //++转圈的速度
    const r = 3 //++旋转半径
    samllball.position.x = Math.sin(time * speed) * r
    samllball.position.z = Math.cos(time * speed) * r //++沿y轴旋转转圈 y轴不动 x z变化  
    samllball.position.y = r + Math.sin(time * speed*r)  //++改变y轴 就会出现上下跳动
