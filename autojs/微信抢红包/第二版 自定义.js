...
device.keepScreenOn(360000000 * 1000); //不息屏操作

var redId = rawInput("请输入红包id", "auf");

var redEnvelopes = id(redId);
...
while (true) {
  if (redEnvelopes.exists()) {
    redEnvelopes_point = id(redId).find();
    if (redEnvelopes_point.length > 0) {
      redEnvelopes_point.forEach((item, index) => {
        redEnvelopes_x = redEnvelopes_point[index].bounds().centerX();
        redEnvelopes_y = redEnvelopes_point[index].bounds().centerY();
