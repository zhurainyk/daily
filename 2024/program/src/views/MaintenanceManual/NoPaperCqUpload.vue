  ...
  <div class="no-paper">
    <div>
      ...
      <div>
        <label>维保单位:</label>
        <el-select
          v-model="data.db_maintCompanyId"
          placeholder=""
          clearable
          filterable
        >
      ...
      <div>
        ...
        <el-button type="primary" @click="searchElevatorClick">绑定</el-button>
      </div>
    </div>
  </div>
</template>
...
import { cqNoPaperApi, cqNoPaperBindApi } from '@/api/module/syscontroller'
...
export default defineComponent({
  setup() {
    ...
    const searchElevatorClick = () => {
      if (!data.time) {
        ElMessage.warning('时间不能为空')
        return
      }
      if (data.projectid) {
        cqNoPaperBindApi(data).then(x => {
          if (x.data.success) {
            ElMessage.success(x.data.message)
          } else {
            ElMessage.error(x.data.message || '失败')
        ...
        ElMessageBox.confirm('是否绑定当前所选时间下所有项目的数据', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消'
        }).then(() => {
          cqNoPaperBindApi(data).then(x => {
