        ...
        <div class="list-body">
          <div
            ...
            :class="[
              {
                'seleced-item':
                  currentProject && currentProject.db_id == x.value
              },
        ...
        <template #searchHead="{ searchBtnList, searchWhere, selectedList }">
          <div class="search-where-head">
            ...
            <div class="search-where">
              ...
              <el-input
                v-model.trim="searchWhere.db_id_username"
                placeholder="姓名"
              />
...
import {
  selectByIdApi as selectProjectByIdApi,
  updateUserIdsByProjectIdApi as updateProjectByIdApi,
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const openAuthList = data => {
      ...
      getUserByProjectIdApi({ db_id: data.value }).then((x: any) => {
        if (x.data.success) {
    ...
    const clickOrg = data => {
      ...
      selectProjectByIdApi({ id: data.value }).then((x: any) => {
        if (x.data.success) {
          //  listRef.value.searchInit()
          currentProject.value = x.data.data
          listRef.value.setselectedList()
          if (currentProject.value.db_userIds) {
            listRef.value.setselectedList(
              currentProject.value.db_userIds.split(',')
            )
          }
        }
      })
    ...
    const addUserToProjectFn = (selecteds: any) => {
      ...
      if (currentProject.value) {
        currentProject.value.db_userIds = tempUserIds
        updateProjectByIdApi({
          userIds: tempUserIds,
          projectId: currentProject.value.db_id
        }).then(x => {
          if (x.data.success) {
            ElMessage({
              message: '新增成功',
