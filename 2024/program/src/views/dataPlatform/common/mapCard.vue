...
const companyType = computed(() => props.companyType)
...
watch(companyId, v => {
    ...
    if (v) {
        parentInfo.length = 1
        setTimeout(() => {
            getGeoJson(100000, { placeCode: '', level: '', companyType: companyType.value, companyId: v }, true) //第三个参数 autoInsert 的作用是用于是否自动进入省市页面
        });
    }
},
    {
...
const getGeoJson = (adcode, params = { placeCode: '', level: '', companyType: companyType.value, companyId: companyId.value }, autoInsert = false) => {
...
const getMapData = async (params, autoInsert) => {
    try {
        ...
        if (autoInsert && cityList.length == 1) { //如果只有一个城市 就直接进入该城市
            ...
            getGeoJson(data.cityCode, { placeCode: data.cityCode, level: data.level, companyType: companyType.value, companyId: companyId.value });
            return
        } else {
            initEcharts(mapData, autoInsert);
        }
...
const initEcharts = (mapData) => {
    ...
    echartsDom.value.setOption({
        ...
        geo: {
            ...
            zoom: 1.15,
            aspectScale: 0.80,
            layoutCenter: ['55%', '45%'], //属性定义地图中心在屏幕中的位置，一般结合layoutSize 定义地图的大小
            layoutSize: 650
        },
    ...
    echartsDom.value.on("click", (params) => {
        ...
        setTimeout(() => {
            ...
            if (data) {
                ...
                getGeoJson(data.cityCode, { placeCode: data.cityCode, level: data.cityLevel, companyType: companyType.value, companyId: companyId.value });
            } else {
                geoJson.features.forEach(item => {
                    if (item.properties.name == params.name) {
                        console.log('点击数据：', { item, parentInfo })
                        //新需求 ： 如果没有项目就不让地图下钻
                        if (!item.properties.projectList?.length) {
                            console.log('没有项目')
                            return
                        }
                        if (lastCode != item.properties.adcode && item.properties.adcode) {
                            ...
                            getGeoJson(item.properties.adcode, { placeCode: item.properties.adcode, level: item.properties.cityLevel, companyType: companyType.value, companyId: companyId.value });
                        }
