                ...
                <div class="card-list">
                    <div class="temp_">
                        <card class="card" sanjiaoColor="#23FFFC" :customeStyle="{ width: '96%' }" borderColor="#068583"
                            ...
                            <span class="value" :class="{ bottomline: borderLine('ProCompany') }"
                                @click="toDetail('ProCompany')">{{ $data.propCompanyCount || 0 }}</span>
                        </card>
                        <card class="card" sanjiaoColor="#23FFFC" :customeStyle="{ width: '96%', }" borderColor="#068583"
                            bg="rgb(18,101,134)">
                            <span class="label">维保单位</span>
                            <span class="value" :class="{ bottomline: borderLine('InsureMaintenanceCompany') }"
                    ...
                    <div class="temp_">
                        <card class="card" sanjiaoColor="#239AFF" :customeStyle="{ width: '96%' }" borderColor="#009AFC"
                            ...
                            <span class="value  " :class="{ bottomline: borderLine('ProjectManagementInsure') }"
                                @click="toDetail('ProjectManagementInsure')">{{ $data.projectCount || 0 }}</span>
                        </card>
                        <card class="card" sanjiaoColor="#239AFF" :customeStyle="{ width: '96%', }" borderColor="#009AFC"
                            bg="rgb(11,52,109)">
                            <span class="label">电梯总数</span>
                            <span class="value  " :class="{ bottomline: borderLine('ElevManageCusotom') }"
                                @click="toDetail('ElevManageCusotom')">{{ $data.elevatorCount || 0 }}</span>
                        </card>
                    </div>
                    <div class="temp_">
...
const toDetail = (e) => {
    if (companyId.value != 'all' && (e == 'InsureMaintenanceCompany' || e == 'ProCompany')) {
        return false
    }
    ...
    if (companyId.value == 'all') {
        ...
        if (e == 'ElevManageCusotom' || e == 'ProjectManagementInsure') {
            params.query.ui_isByCompany = '03'
        }
    } else {
        params.query.db_insuCompanyId = companyId.value
        if (e == 'ElevManageCusotom' || e == 'ProjectManagementInsure') {
            params.query.ui_isByCompany = '03'
        }
    }
