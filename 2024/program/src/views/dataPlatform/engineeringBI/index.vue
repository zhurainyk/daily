...
const getProjectAPI = (noloading = {}) => { // 获取项目
    getCompanyList({ companyType: '01' }).then(res => {
        ...
        if (res.data.data?.length) {
            ...
            $store.commit('CHANGEBIPRO',{key:companyKey,value:currentProjectId.value})
            window.$engCompany = res.data.data
...
onMounted(() => {
    ...
    $bus.on('alarm_engineering', getAlarm)
})
let alarmTimer
let alarmData = {}
...
const autoRun = () => {
    $bus.emit('alarm_engineering_toast', alarmData)
}
