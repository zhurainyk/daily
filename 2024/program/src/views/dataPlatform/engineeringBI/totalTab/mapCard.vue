...
watch(companyId, v => {
    if (v) {
        setTimeout(() => {
            getGeoJson(100000, { placeCode: '', level: '', companyType: '03', companyId: v })
        });
    }
},
    {
...
const getGeoJson = (adcode, params = { placeCode: '', level: '', companyType: '03', companyId: companyId.value }) => {
    ...
    echartsDom.value.setOption({
        ...
        tooltip: {
            ...
            formatter: p => {
                ...
                let txtCon = `
                    <div style=' padding:5px 20px'>所在城市 : <span style='color:#23FFFC'>${p.name}</span> </div>
