                ...
                <div class="card-list">
                    <div class="temp_">
                        <card class="card" sanjiaoColor="#23FFFC" :customeStyle="{ width: '96%' }" borderColor="#068583"
                            ...
                            <span class="value" :class="{ bottomline: toDetailB('ProCompany') }"
                                @click="toDetail('ProCompany')">{{
                                    $data.propCompanyCount || 0 }}</span>
                        </card>
                        <card class="card" sanjiaoColor="#23FFFC" :customeStyle="{ width: '96%', }" borderColor="#068583"
                            bg="rgb(18,101,134)">
                            <span class="label">维保单位</span>
                            <span class="value  " :class="{ bottomline: toDetailB('InsureMaintenanceCompany') }"
                                @click="toDetail('InsureMaintenanceCompany')">{{
                                    $data.maintConpanyCount || 0 }}</span>
                        </card>
                    </div>
                    <div class="temp_">
                        <card class="card" sanjiaoColor="#239AFF" :customeStyle="{ width: '96%' }" borderColor="#009AFC"
                            ...
                            <span class="value  " :class="{ bottomline: toDetailB('ProjectManagementInsure') }"
                        ...
                        <card class="card" sanjiaoColor="#239AFF" :customeStyle="{ width: '96%', }" borderColor="#009AFC"
                            ...
                            <span class="value  " :class="{ bottomline: toDetailB('ElevManageCusotom') }"
...
const toDetail = (e) => {
    if (companyId.value != 'all' && e == 'ProCompany') {
        return
    }
    ...
    if (companyId.value == 'all') {
        ...
        if (e == 'ElevManageCusotom' || e == 'ProjectManagementInsure') {
            params.query.ui_isByCompany = '02'
        }
    } else {
        if (e == 'InsureMaintenanceCompany') {

            params.query.db_id_companyName = window?.$engCompany?.find(x => x.id == companyId.value)?.name || ''
        } else {
            params.query.db_maintCompanyId = companyId.value
        }
        if (e == 'ElevManageCusotom' || e == 'ProjectManagementInsure') {
            params.query.ui_isByCompany = '02'
        }
