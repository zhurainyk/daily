<template>
    <border class="border">
        <div class="temp">
            <module-name :moduleName="'实时信息'"></module-name>
            <div class="content">
                <el-scrollbar height="100%">
                    <div class="item" v-for="(item, index) in tablelist" @click="editDetailFn(item)" :key="item.orderId">
                        <div class="left">
                            <div class="label"
                                :style="{ color: item.color, borderColor: item.color, backgroundColor: item.bgc }">
                                {{ item.orderTypeDesc }}</div>
                            <div class="value" :title="item.orderContent"> {{ item.orderContent }}</div>
                        </div>
                        <div class="right">
...
let heartData = { "socketType": "02", "keyline": "" }
watch(companyId, v => {
    if (v) {
        // initSocket(v)
        heartData.keyline = v
...
const initSocket = () => {
    setTimeout(() => {
        ...
        useWebSocketRT(
            `${socketUrl}/elevator_abeyance/websocket/bi?token=${token.value}`, // url
            null, // onopen回调
            (e) => { // onmessage回调
                ...
                switch (res.data?.socketType) {
                    case '02': {
                        if (res.data.data) {
                            let repeatMark = res.data.data.date + res.data.data.time + res.data.data.elevatorId + res.data.data.projectId + res.data.data.eventName
                            ...
                            tablelist.value.unshift(res.data.data)
                        }
                        break;
                    }
                }
