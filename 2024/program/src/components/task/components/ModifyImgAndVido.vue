...
export default defineComponent({
  ...
  setup(props, { emit }) {
      ...
      if (videoslist.value && videoslist.value.url) {
        addVideoByIdApi({
          ...
          if (x.data.success) {
            ...
            props.baseInfo.db_videoUrl = addfileslist.value
              .map(x => x.url)
              .join(',')
          } else {
            ElMessage.error('失败')
    ...
    const removeimg = val => {
      let temp = addimgslist.value.map(x => x.url).join(',')
      props.baseInfo.db_imgUrl = temp
    }
    ...
    const save = () => {
      ...
      addImgVideoApi({
        imgUrls: addimgslist.value.map(x => x.url).join(','),
        videoUrls: addComment.db_filesUrls,
        id: props.baseInfo.db_id
      }).then(x => {
        if (x.data.success) {
          props.baseInfo.db_videoUrl = addComment.db_filesUrls
          props.baseInfo.db_imgUrl = addComment.db_imagesUrls
          ElMessage.success('成功')
          closeDialogFn()
        } else {
    ...
    onMounted(() => {
      if (props.baseInfo) {
        if (props.baseInfo.db_imgUrl) {
          addimgslist.value = props.baseInfo.db_imgUrl
            .split(',')
        ...
        if (props.baseInfo.db_videoUrl) {
          addfileslist.value = props.baseInfo.db_videoUrl
            .split(',')
            .map((x, index) => {
              const arr = x.split('/')
              return {
        ...
        if (props.baseInfo.db_videoRecords) {
          db_videoRecords.value = JSON.parse(props.baseInfo.db_videoRecords)
        }
        searchTime.elevatorId = props.baseInfo.db_elevId
